...
export const LANE_DEFINITIONS = {
  check: {
    ...
    command: [
      "bash",
      "-lc",
      "python3 scripts/local_gate_plan.py --base-ref ${LOCAL_GATE_BASE_REF:-origin/main} --head-ref HEAD --fail-on-unmapped && python3 scripts/check_pipeline_contract.py && DOCS_BUNDLE_BASE_SHA=${DOCS_BUNDLE_BASE_SHA:-origin/main} python3 scripts/check-endpoint-doc-bundle.py && scripts/check-docs-nav.py && scripts/check-docs-links.py && scripts/check-route-doc-parity.py && scripts/check-cgs-route-doc-parity.py && scripts/check-env-doc-parity.py && python3 scripts/check-announcement-dm-guard.py && python3 scripts/check-optional-service-guards.py && python3 scripts/check-windows-powershell-compat.py && python3 scripts/check-qdrant-scope-guard.py",
    ],
  },
  lint: {
