...
from zetherion_ai.profile.models import ProfileCategory, ProfileEntry
...
log = get_logger("zetherion_ai.profile.cache")
...
class ProfileCache:
    ...
    def __init__(
        ...
        full_employment_cache_ttl: int = 900,
        relationship_cache_ttl: int = 900,
        max_users: int = 100,
    ):
        ...
        Args:
            ...
            full_employment_cache_ttl: TTL for full EmploymentProfile in seconds (default 15 min).
            relationship_cache_ttl: TTL for RelationshipTracker in seconds (default 15 min).
        ...
        # Full EmploymentProfile objects (Phase 5C.1)
        self._full_employment_cache: TTLCache[str, EmploymentProfile] = TTLCache(
            maxsize=max_users, ttl=full_employment_cache_ttl
        )

        # RelationshipTracker objects (Phase 5C.1)
        self._relationship_cache: TTLCache[str, RelationshipTracker] = TTLCache(
            maxsize=max_users, ttl=relationship_cache_ttl
        )
        ...
        log.info(
            ...
            full_employment_ttl=full_employment_cache_ttl,
            relationship_ttl=relationship_cache_ttl,
            max_users=max_users,
        )
    ...
    def get_full_employment_profile(self, user_id: str) -> EmploymentProfile | None:
        """Get cached full EmploymentProfile.

        Args:
            user_id: The user's ID.

        Returns:
            Full EmploymentProfile, or None if not cached.
        """
        return self._full_employment_cache.get(user_id)  # type: ignore[no-any-return]

    def set_full_employment_profile(self, user_id: str, profile: EmploymentProfile) -> None:
        """Cache full EmploymentProfile.

        Args:
            user_id: The user's ID.
            profile: Full EmploymentProfile.
        """
        self._full_employment_cache[user_id] = profile
        log.debug("full_employment_profile_cached", user_id=user_id)

    def get_relationship_tracker(self, user_id: str) -> RelationshipTracker | None:
        """Get cached RelationshipTracker.

        Args:
            user_id: The user's ID.

        Returns:
            RelationshipTracker, or None if not cached.
        """
        return self._relationship_cache.get(user_id)  # type: ignore[no-any-return]

    def set_relationship_tracker(self, user_id: str, tracker: RelationshipTracker) -> None:
        """Cache RelationshipTracker.

        Args:
            user_id: The user's ID.
            tracker: RelationshipTracker instance.
        """
        self._relationship_cache[user_id] = tracker
        log.debug("relationship_tracker_cached", user_id=user_id)
    ...
    def invalidate(self, user_id: str) -> None:
        ...
        self._full_employment_cache.pop(user_id, None)
        self._relationship_cache.pop(user_id, None)
    ...
    def invalidate_all(self) -> None:
        ...
        self._full_employment_cache.clear()
    ...
    def stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "full_employment_cache_size": len(self._full_employment_cache),
            "full_employment_cache_max": self._full_employment_cache.maxsize,
            "relationship_cache_size": len(self._relationship_cache),
            "relationship_cache_max": self._relationship_cache.maxsize,
        }
