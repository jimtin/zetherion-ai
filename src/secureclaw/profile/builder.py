...
from secureclaw.profile.inference import ProfileInferencePipeline
...
from secureclaw.profile.storage import ProfileStats, ProfileStorage

if TYPE_CHECKING:
    from secureclaw.agent.inference import InferenceBroker
    from secureclaw.memory.qdrant import QdrantMemory

log = get_logger("secureclaw.profile.builder")
...
class ProfileBuilder:
    ...
    async def get_employment_profile(self, user_id: str) -> EmploymentProfileSummary:
        """Get the employment profile for a user.
        ...
        self._cache.set_employment_profile(user_id, profile)
        return profile
