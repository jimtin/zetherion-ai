...
16. Shared trust/personal/profile/portfolio/routing/queue/model/context/storage/startup changes mapped in `.ci/local_gate_manifest.json` must run the bounded `unit-full` lane before push, and mapped CI-support/workflow/deploy-preflight changes must run their targeted regression suites before push.
