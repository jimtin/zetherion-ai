...
def test_docker_runtime_exposes_non_throwing_wsl_helpers() -> None:
    ...
    assert "zetherion-wsl-runtime-mount-" in docker_runtime
    assert "zetherion-wsl-runtime-path-" in docker_runtime
    assert 'mount_line=$(mount | grep " on $drive_root " | head -n 1 || true)' in docker_runtime
    assert "$targetWindowsPath = Join-Path $DeployPath $relativePath" in docker_runtime
    assert "$targetWslPath = ConvertTo-ZetherionWslPath -WindowsPath $targetWindowsPath" in docker_runtime
    assert 'touch "$target/.wsl-write-check"' in docker_runtime
    assert 'Invoke-ZetherionWslCommandResult -User "root" -Command ("bash " +' in docker_runtime
    ...
    assert "function Get-ZetherionDockerDesktopStartupTaskStatus" in docker_runtime
    ...
    assert "function Test-ZetherionScheduledTaskActionContains" in docker_runtime
    ...
    assert "function Start-ZetherionDockerDesktopViaScheduledTask" in docker_runtime
